...
-- 05_raw_tables_copy.sql.
--
-- Airflow impact: once this is wired up, the ingestion_dag's copy_into
-- task (airflow/dags/ingestion_dag.py) is no longer needed - dlt's
-- extract() writes to S3 and the pipe loads it on its own. reconcile()
-- can no longer assume the row landed the instant extract() returns
-- (Snowpipe is async, typically seconds-to-low-minutes) - point it at
-- COPY_HISTORY / PIPE_USAGE_HISTORY, or give it a short poll, instead of
-- counting rows immediately after extract.
...
--
-- No IAM policy change needed for the queue itself: Snowflake owns the SQS
-- queue behind notification_channel and its access policy already allows
-- this S3 bucket (via the storage integration's trust relationship) to
-- publish to it.
